...
async function seed() {
  ...
  // Branches — idempotent via ON CONFLICT DO NOTHING
  ...
  const b1 = branch1 ?? existingBranches[0];
  const b2 = branch2 ?? existingBranches[1];
  console.log("Branches seeded");

  // Users — idempotent via ON CONFLICT DO NOTHING (unique on phone)
  ...
  // Menu Items — only insert if we just created the categories
  if (catMain) {
  ...
  // Customers — idempotent via ON CONFLICT DO NOTHING (unique on phone)
